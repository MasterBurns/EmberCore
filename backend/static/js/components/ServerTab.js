        ...
        <div class="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div class="flex-1 min-w-0 w-full space-y-6">
                ...
                <div v-if="store.serverTab === 'status'" class="space-y-6">
                    ...
                    <div v-if="store.serverStats?.update_info?.available" class="bg-blue-950/80 border border-blue-500 rounded-xl p-5 shadow-[0_0_15px_rgba(59,130,246,0.3)] flex justify-between items-center animate-pulse">
                        <div>
                        ...
                        <div class="bg-gray-950 p-5 rounded-xl border border-gray-900 shadow-inner flex flex-col justify-between">
                            ...
                            <div class="flex justify-between items-end">
                                <div>
                                    ...
                                    <p class="text-3xl font-mono font-bold text-orange-400">{{ store.serverStats?.status === 'online' ? (store.serverStats?.cpu_percent || 0) + ' %' : '---' }}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-[10px] text-gray-600 font-bold uppercase">RAM-Verbrauch</p>
                                    <p class="text-3xl font-mono font-bold text-blue-400">{{ store.serverStats?.status === 'online' ? (store.serverStats?.ram_mb || 0) + ' MB' : '---' }}</p>
                                </div>
                            </div>
                        </div>
                ...
                <div v-if="store.serverTab === 'config'" class="bg-gray-950 rounded-xl p-5 border border-gray-900 shadow-md space-y-4">
                    ...
                    <div v-if="store.startupData?.enabled" class="mt-4 p-4 bg-gray-900/40 border border-gray-800 rounded-lg shadow-inner">
                ...
                <div v-if="store.serverTab === 'lists'" class="space-y-6">
                    <div v-if="!store.listData?.enabled" class="bg-gray-950 rounded-xl p-8 border border-gray-900 text-center text-gray-500 text-sm shadow-md">
                        Dieses Spiel unterstützt noch keine Textlisten im Webinterface.
                    </div>
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div v-for="lst in store.listData?.lists || []" :key="lst.id" class="bg-gray-950 rounded-xl p-5 border border-gray-900 shadow-md flex flex-col h-96">
                            <div class="flex justify-between items-center mb-2">
                                <h3 class="text-sm font-bold text-white uppercase">{{ lst.name }}</h3>
                                <button @click="api.saveList(lst)" class="bg-orange-600 hover:bg-orange-500 text-white text-[10px] font-bold px-3 py-1.5 rounded cursor-pointer transition">SPEICHERN</button>
                            </div>
                ...
                <div v-if="store.serverTab === 'network'" class="space-y-6">
                    <div class="bg-gray-950 rounded-xl p-6 border border-gray-900 shadow-md space-y-4">
                        <div>
                            <h3 class="text-lg font-black text-white uppercase tracking-wider">🌍 Netzwerk-Ports & Freigaben</h3>
                            <p class="text-sm text-gray-400 mt-1">Damit das Spiel aus dem Internet erreichbar ist, müssen die Ports deines PCs und Routers geöffnet sein.</p>
                        </div>
                        <div v-if="!store.networkData?.enabled" class="text-gray-500 text-sm py-4 border-t border-gray-900">
                            Keine Ports für dieses Spiel im Manifest konfiguriert.
                        </div>
                    ...
                    <div class="bg-gray-950 rounded-xl border border-gray-900 overflow-hidden shadow-md">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-gray-900 text-xs text-gray-300">
                                ...
                                <tr v-if="!store.activeMods?.length">
                                    <td colspan="4" class="p-6 text-center text-gray-600">Keine Modifikationen aktiv.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div v-if="store.serverTab === 'backups'" class="space-y-6">
                    <div class="bg-gray-950 rounded-xl p-5 border border-gray-900 shadow-md space-y-4">
                        <div class="flex justify-between items-center border-b border-gray-900 pb-3">
                            <div><h3 class="text-sm font-bold text-white uppercase tracking-wider">🛡️ Speicherplatz-Optimierung</h3></div>
                            <button @click="api.saveBackupSettings()" class="bg-gray-900 hover:bg-gray-800 border border-gray-800 text-xs font-semibold px-4 py-2 rounded-lg text-gray-300 transition cursor-pointer">Speichern</button>
                        </div>
                        <div class="space-y-3 pt-2">
                            <div class="flex items-center justify-between bg-gray-900/50 p-3 rounded-lg border border-gray-800">
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-500 mt-1">Löscht niemals die neuesten X Sicherungen.</p>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <input type="number" v-model.number="store.backupSchedule.retention.keep_latest" class="w-16 bg-gray-950 border border-gray-700 rounded p-1.5 text-sm text-white text-center font-mono outline-none">
                                    <span class="text-xs text-gray-400 w-12">Stück</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between bg-gray-900/50 p-3 rounded-lg border border-gray-800">
                                <div>
                                    <p class="text-sm font-bold text-white">Tägliche Backups aufheben</p>
                                    <p class="text-[10px] text-gray-500 mt-1">Archiviert das erste Backup jedes Tages.</p>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <input type="number" v-model.number="store.backupSchedule.retention.keep_daily" class="w-16 bg-gray-950 border border-gray-700 rounded p-1.5 text-sm text-white text-center font-mono outline-none">
                                    <span class="text-xs text-gray-400 w-12">Tage</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-950 rounded-xl p-5 border border-gray-900 shadow-md space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center space-x-2 border-t border-gray-900 pt-3">
                                <select v-model="store.newSchedType" class="bg-gray-900 border border-gray-800 text-xs text-white p-2 rounded outline-none w-32">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-900/40 border-b border-gray-900 text-gray-500 text-[10px] uppercase font-bold">
                                    ...
                                    <th class="p-3 text-right">Größe</th>
                                    <th class="p-3 text-right">Aktionen</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-900 text-xs text-gray-300">
                                ...
                                <tr v-if="!store.backupList?.length">
                                    <td colspan="3" class="p-6 text-center text-gray-600">Keine Backups vorhanden.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
